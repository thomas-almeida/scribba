 ...
 * @param {PromptContent} content - The content object from the prompt
 * @param {string} title - The title of the prompt
 * @returns {string} - Formatted text
 */
const formatPromptContent = (content: PromptContent, title: string): string => {
 ...
 * @returns {Promise<ApiResponse>} - The API response with generated text
 */
const createSEOPrompt = async (keyword: string, promptId: number = 0): Promise<ApiResponse> => {
  // Find the prompt with the given ID
  const promptData = (promptsData as PromptData[]).find(prompt => prompt.id === promptId);
 ...
 */
const convertToMarkdown = async (text: string): Promise<string> => {
  const md = {
    model: "meta-llama/llama-4-maverick:free",
    messages: [
      {
 ...
 * @returns {Promise<ProcessKeywordResult>} - Object containing results of all three steps
 */
const processKeyword = async (keyword: string, promptId: number = 0): Promise<ProcessKeywordResult> => {
  try {
...
export { createSEOPrompt, convertToMarkdown, processKeyword };

/**
 * Saves a liked text to the browser's localStorage
 ...
 */
export const saveLikedText = async (keyword: string, markdownText: string, jsonStructure: any) => {
  try {
    // Get existing saved texts from localStorage
    const savedTextsJSON = localStorage.getItem('likedTexts') || '[]';
    const savedTexts = JSON.parse(savedTextsJSON);
    ...
    // Add the new entry to the saved texts
    savedTexts.push(newEntry);
    
    // Save back to localStorage
    localStorage.setItem('likedTexts', JSON.stringify(savedTexts));
    
    return { success: true, savedTexts, newEntry };
  } catch (error) {
...
/**
 * Gets all liked texts from localStorage
 * @returns {Promise<Array>} - Array of all saved texts
 */
export const getLikedTexts = async () => {
  try {
    const savedTextsJSON = localStorage.getItem('likedTexts') || '[]';
    return JSON.parse(savedTextsJSON);
  } catch (error) {
